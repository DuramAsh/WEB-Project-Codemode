...
export class AccountComponent implements OnInit {
  ...
  async postComment() {
    ...
    const { value: course_id } = await Swal.fire({
      ...
      html:
        '<textarea type="text" id="textarea" class="swal2-input">',
      inputPlaceholder: 'Выберите курс',
      showCancelButton: true,
      
    })

    this.text = (<HTMLTextAreaElement>document.getElementById("textarea")).value;
    ...
    this.service.postComment(+id, course_id, this.text).subscribe(response => {
      if(response != null){
        this.Toast.fire({
          icon: 'success',
